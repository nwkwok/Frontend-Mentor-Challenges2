...
var jobCard = document.getElementsByClassName('job-card-container'); // Arrays
...
for (var _i = 0; _i < jobFilters.length; _i++) {
  jobFilters[_i].addEventListener('click', function (e) {
    ...
    selectedFilters.innerHTML += "\n        <span>".concat(e.target.innerText, "</span>\n        <img id=\"remove\" class=\"remove\" src=\"images/icon-remove.svg\"></img>"); //Toggle filter box

    filterArr.length !== 0 && filterBox.classList.remove('hidden'); // Loop through each card and create an array with that card's filter values

    for (var _i2 = 0; _i2 < jobCard.length; _i2++) {
      var cardArr = [];
      cardArr.push.apply(cardArr, [data[_i2].role, data[_i2].level].concat(_toConsumableArray(data[_i2].languages))); //Once you have the card value, check it to filterArr
